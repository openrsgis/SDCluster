...
class CustomRandomHorizontalFlip(nn.Module):
    ...
    def __call__(self, multi_crops, gc_bboxes, otc_bboxes):
        flags = []
        gc_bboxes_clone = gc_bboxes.clone()
        otc_bboxes_clone = otc_bboxes.clone()
        for i in range(len(multi_crops)):
            flag = False
            if torch.rand(1) < self.p:
                multi_crops[i] = TF.hflip(multi_crops[i])
                if i < 2:
                    gc_bboxes[i, :, 0] = 1.0 - gc_bboxes_clone[i, :, 2]
                    gc_bboxes[i, :, 2] = 1.0 - gc_bboxes_clone[i, :, 0]
                otc_bboxes[i, :, 0] = 1.0 - otc_bboxes_clone[i, :, 2]
                otc_bboxes[i, :, 2] = 1.0 - otc_bboxes_clone[i, :, 0]
                flag = True
            flags.append(flag)
...
class LeopartTransforms:
    ...
    def __init__(self,
        ...
        self.color_jitter = torchvision.transforms.ColorJitter(
            0.4 * jitter_strength, 0.4 * jitter_strength, 0.2 * jitter_strength,
            0.1 * jitter_strength
        )
    ...
    def __call__(self, sample: torch.Tensor) -> Tuple[List[Tensor], Dict[str, Tensor]]:
        ...
        gc_bboxes = gc_bboxes / 224.0
        otc_bboxes[:2] = otc_bboxes[:2] / 224.0
        otc_bboxes[2:] = otc_bboxes[2:] / 96.0

        multi_crops, gc_bboxes, otc_bboxes, flags = self.hflip(multi_crops, gc_bboxes, otc_bboxes)  ##################

        for i in range(len(multi_crops)):
            multi_crops[i] = self.final_transform(multi_crops[i])
        return multi_crops, gc_bboxes, otc_bboxes, flags
